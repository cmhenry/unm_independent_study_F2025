...
political_corpus <- generate_sample_corpus(100)  # Larger corpus for embeddings
...
predict(w2v_model, c("democracy"), type = "nearest", top_n = 10)
...
concepts <- c("democracy", "security", "economy", "healthcare")
...
similar_to_concepts <- lapply(concepts, function(word) {
  if (word %in% rownames(embeddings)) {
    ...
    tibble(
      concept = word,
      similar_word = sims$term2,
      similarity = sims$similarity
    )
  } else {
...
# Visualize
similar_to_concepts %>%
  filter(!is.na(similar_word)) %>%
  ggplot(aes(x = reorder(similar_word, similarity), y = similarity, fill = concept)) +
  geom_col() +
  coord_flip() +
...
word_pairs <- tribble(
  ...
  "democracy", "autocracy", "Medium",
  "democracy", "banana", "Low",
  "government", "regime", "High",
  "economy", "healthcare", "Medium",
  "election", "voting", "High"
)
...
## 5.1 The Analogy Task

One of the most famous properties of embeddings: **vector arithmetic captures semantic relationships**.

**Classic example:** "king" - "man" + "woman" ≈ "queen"

**Political example:** "democracy" - "elections" + "control" ≈ "autocracy"

## 5.2 Performing Analogies
...
## 5.3 Exploring Political Analogies
...
**Discussion:** Do these analogies make substantive sense? This is where Rodriguez & Spirling's validation insights matter!
